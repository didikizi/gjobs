...
func (cs *cronScheduler) register(name, schedule string) error {
	...
	return cs.storage.UpsertCron(context.Background(), entry)
}
...
func (cs *cronScheduler) fire() {
	...
	if err != nil {
		cs.logger.Error("cron poll: %v", err)
		return
	}
	...
	for _, entry := range due {
		...
		if err := cs.storage.UpdateCronRun(ctx, entry.Name, now, next); err != nil {
			cs.logger.Error("update cron run (name=%s): %v", entry.Name, err)
			continue
		}

		if err := cs.enqueue(entry.Name); err != nil {
			cs.logger.Error("enqueue cron job (name=%s): %v", entry.Name, err)
		}
	}
}
