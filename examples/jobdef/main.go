...
func main() {
	...
	jobs.HandleDef[Email](q, CriticalEmail, func(ctx context.Context, e Email) error {
		fmt.Printf("  [CRITICAL] → %s: %q\n", e.To, e.Subject)
		return nil
	})

	jobs.HandleDef[Payment](q, ChargeCard, func(ctx context.Context, p Payment) error {
		fmt.Printf("  [charge]  → user=%s $%.2f\n", p.UserID, p.AmountUSD)
		return nil
	})

	jobs.HandleDef[ReportParams](q, GenerateReport, func(ctx context.Context, r ReportParams) error {
